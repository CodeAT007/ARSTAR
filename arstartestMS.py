...
def plot(values1, values2):
    ...
    plt.loglog(values2.keys(),values2.values(),'yo',basex=10,basey=10, label ='MergeSort Builtin')
    plt.legend(loc=2)
...
def benchmark(MaxN,TestN=100):
    ...
    while size <= MaxN:
        ...
        #Dictionaries to hold average runnign times (code, MergeSort Builtin)
        temp1 = []
        temp2 = []
        ...
        for i in range(TestN):
            ...
            sorted_list_MS = list(unsorted_list)
            ...
            #Sort Items MergeSort builtin
            start = time.clock()
            mergesort(sorted_list_MS) #Sort Items
            elap = time.clock() - start
            temp2.append(elap)
